          ...
          (context) => AddTodoDialog(
            ...
            onSubmit: (title, description, priority) {
              if (todo == null) {
                // Create a new Todo
                final todos =
                    context.read<TodoBloc>().state is TodoLoaded
      ...
      body: BlocConsumer<TodoBloc, TodoState>(
        ...
        builder: (context, state) {
          if (state is TodoLoading) {
            ...
            return Row(
              children:
                  ['Chưa làm', 'Đang làm', 'Hoàn thành'].map((status) {
                    return Expanded(
                      child: StatusColumn(
                        status: status,
                        todos: todos,
                        onEdit:
                            (todo) => _showAddEditDialog(context, todo: todo),
                        onDelete: (todo) => _deleteTodo(context, todo),
                        onReorder:
                            (oldIndex, newIndex) => _updateTodoPositions(
                              context,
                              oldIndex,
                              newIndex,
                              status,
